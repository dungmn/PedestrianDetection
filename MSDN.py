...
raw_RRN = MyRRN()
raw_RRN.load_state_dict(torch.load('mymodel/RRN/model23_lr_1e-9_bz_6_NBS_128_norm_epoch_3.ptx'))
...
def train(pretrain):
    ...
    f = open('models/MSDN/model14/log14_163.txt','a')
...
def test(pretrain):
    ...
    test_file = 'mymodel/MSDN/test1/test163_{}.txt'.format(pretrain.split('/')[-1])
    f = open(test_file,'a')
    with torch.no_grad():
        for i, sample in enumerate(dataloader):
...
if __name__ == '__main__':
    args = parse_args()
    pretrain = 'models/MSDN/model14/model14_lr_1e-4_bz_2_decay_epoch_2.pth'
